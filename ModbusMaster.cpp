...
@ingroup setup
*/
void ModbusMaster::begin(uint8_t slave, BufferedSerial &serial)
{
...
uint8_t ModbusMaster::ModbusMasterTransaction(uint8_t u8MBFunction)
{
  ...
  for (i = 0; i < u8ModbusADUSize; i++)
  {
    ...
    char writebuf[u8ModbusADUSize];
    writebuf[i] = u8ModbusADU[i];
    _serial->write(writebuf, u8ModbusADUSize);
  }
  ...
  while (u8BytesLeft && !u8MBStatus)
  {
    if (_serial->readable())
    {
      ...
      char readbuf[1];
      _serial->read(readbuf, 1);
      u8ModbusADU[u8ModbusADUSize++] = (uint8_t)atoi(readbuf);
      u8BytesLeft--;
#if __MODBUSMASTER_DEBUG__
